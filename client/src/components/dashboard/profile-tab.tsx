...
export function ProfileTab({
  ...
  const inputClass = "rounded-full h-11 px-5 border-2 border-border dark:border-border focus:border-accent bg-white dark:bg-muted transition-colors font-medium text-foreground text-sm";
  const selectClass = "rounded-full h-11 px-5 border-2 border-border dark:border-border bg-white dark:bg-muted";
  const readOnlyClass = "p-2.5 px-5 bg-muted dark:bg-muted rounded-full text-sm h-11 flex items-center";
  ...
  return (
    <div key="profile" className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <div className="p-2.5 px-5 bg-muted dark:bg-muted rounded-full flex justify-between items-center text-sm h-11">
          ...
          {isEditing && canEdit && (
            <div className="mt-4 space-y-2">
              <Button onClick={() => updateProfile.mutate(profileData)} className="w-full bg-accent text-accent-foreground font-black rounded-full h-11 transition-colors" disabled={updateProfile.isPending} data-testid="button-save-profile">{updateProfile.isPending ? t('common.saving') : t('profile.saveChanges')}</Button>
            </div>
          )}
        </div>
      </Card>
      ...
      <Card className={`rounded-2xl shadow-sm p-5 bg-white dark:bg-card transition-opacity duration-200 ${(profileOtpStep === 'otp' || hasPendingChanges) ? 'opacity-50 pointer-events-none select-none' : ''}`}>
        ...
        {showPasswordForm && (
          <div className="space-y-3 p-4 bg-muted dark:bg-muted rounded-xl">
            {passwordStep === 'form' && (
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="space-y-1">
