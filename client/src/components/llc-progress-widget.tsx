...
export function LLCProgressWidget({ status, serviceName, state, requestCode, isMaintenance = false }: ProgressWidgetProps) {
  ...
  return (
    <Card className={`overflow-hidden max-w-sm sm:max-w-none mx-auto sm:mx-0 ${status === 'completed' ? 'border-green-500 dark:border-green-600' : ''}`} data-testid={`widget-progress-${isMaintenance ? 'maintenance' : 'llc'}`}>
