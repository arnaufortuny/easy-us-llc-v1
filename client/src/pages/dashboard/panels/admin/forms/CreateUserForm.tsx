  ...
  return (
    <Card className="mb-4 p-4 md:p-6 rounded-2xl border border-accent/30 bg-white dark:bg-card shadow-lg animate-in slide-in-from-top-2 duration-200">
      <div className="flex items-center justify-between mb-4">
        ...
        <Button variant="ghost" size="icon" onClick={onClose} className="rounded-full" data-testid="button-close-create-user" type="button">
          <X className="w-4 h-4" />
        </Button>
      </div>
      <form noValidate onSubmit={(e) => { e.preventDefault(); createUserMutation.mutate(newUserData); }}>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-6 pt-4 border-t">
          <Button type="submit" disabled={createUserMutation.isPending || !newUserData.email || !newUserData.password} className="flex-1 bg-accent text-accent-foreground font-black rounded-full" data-testid="button-confirm-create-user">
            {createUserMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : t('dashboard.admin.createClient')}
          </Button>
          <Button variant="outline" type="button" onClick={onClose} className="flex-1 rounded-full" data-testid="button-cancel-create-user">{t('common.cancel')}</Button>
        </div>
      </form>
    </Card>
  );
